"""Pathwise transforms that avoid the source/target overlap of TransformFromCopy."""
...
class CopyMorph(Scene):
    def construct(self) -> None:
        ...
        for target, target_color in zip(poses[1:], colors[1:]):
            self.play(
                LaggedStart(
                    *(Transform(source, destination) for source, destination in zip(current, target)),
                    lag_ratio=0.035,
                ),
